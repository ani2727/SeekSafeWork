...
function Header() {
  ...
  const handleMarkAllAsRead = () => {
    fetch(`http://127.0.0.1:4000/clearNotificatoins/${userInfo.id}`).then(
      (res) => {
        res.json().then((data) => {
          console.log(data);
          setNotifications([]);
  ...
  React.useEffect(() => {
    if (!isLoading) {
      fetch(`http://127.0.0.1:4000/notification/${userInfo?.id}`)
        .then((res) => res.json())
        .then((data) => {
          if (data.unviewed) setNotifications(data.unviewed); // Set count to the value received from the API
        })
        .catch((error) => {
                          ...
                          <React.Fragment key={work.id}>
                            ...
                            >
                              <Avatar
                                ...
                                sx={{
                                  bgcolor: getAvatarColor(userName[0]),
                                  m: "2px",
                                }}
                              >
                              ...
                              <Typography sx={{ color: "black" }}>
                                ...
                                is needed at our Location. Have a look at this
                                work.
                              </Typography>
                            </Box>
              ...
              <Box sx={{ flexGrow: 0 }}>
                  ...
                  <IconButton onClick={handleOpenUserMenu} sx={{ p: 0 }}>
                    {userName ? (
                      <Avatar
                        alt={userName}
                        src="/static/images/avatar/2.jpg"
                        sx={{ bgcolor: getAvatarColor(userName[0]) }}
                      >
                        {userName[0].toUpperCase()}{" "}
                      ...
                      <Avatar sx={{ width: 40, height: 40 }}>
                        ...
                        {/* Adjusting the size of AccountCircleIcon */}
                      </Avatar>
                    )}
                  </IconButton>
                </Tooltip>
                ...
                >
                  {userName ? ( // Conditional rendering based on userInfo
                    <>
                      <MenuItem component={Link} to="/workpost">
