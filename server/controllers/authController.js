...
module.exports.myBookmarks_get = async (req, res) => {
  try {
    ...
    const Data = await Bookmark.findOne({ author: id }).populate({
    ...
    if (!Data) {
      return res
        .status(404)
        .json({ error: "No bookmarks found for the author" });
    }
    res.json(Data.saved);
  } catch (error) {
...
module.exports.clearNotificatoins = async (req, res) => {
  ...
  try {
    ...
    if (!userNotifications || !userNotifications.unviewed) {
      console.log(
        "No notifications found or unviewed notifications array is empty."
      );
      return res.json(userNotifications.unviewed); // Sending response with info and count
    }
...
module.exports.sendOtp_get = async (req, res) => {
  ...
  if (!isValidEmail(email)) {
    return res
      .status(400)
      .json({ errors: { email: "Please provide a valid email" } });
  }
  ...
  try {
    ...
    if (isExist) {
      return res.status(400).json({
        errors: {
          email: "This email is already registered. Try with a different one.",
        },
      });
    }
